...
*
* @section DS1631license GNU General Public License v3.0
*
...
* ------ | ---------- | ----------------------------- | --------
* 1.0.4  | 2019-01-26 | https://github.com/SV-Zanshin | Issue #9 - converted documentation to doxygen
...
*******************************************************************************************************************/
  class DS1631_Class
  {
    public:
      ...
      int16_t  readTemp(const uint8_t device=0);
      void     setPrecision(const uint8_t device,const uint8_t precision);
      void     setAlarmTemperature(const uint8_t device,const uint8_t alarmType,int16_t alarmTemp);
      uint8_t  getAlarm(const uint8_t device);
      void     setPolarity(const uint8_t device, const bool polarity);
      void     setContinuous(const uint8_t device,const bool continuous=true);
      void     reset(const uint8_t device);
    private:
      ...
      void     writeCommand(const uint8_t device, const uint8_t command);
      uint8_t  readByte    (const uint8_t device, const uint8_t command);
      uint16_t readWord    (const uint8_t device, const uint8_t command);
      uint8_t  writeByte   (const uint8_t device, const uint8_t command,const uint8_t data);
      uint8_t  writeWord   (const uint8_t device, const uint8_t command,const int16_t data);
