...
/***************************************************************************************************************//*!
* @brief     Starts I2C communications and search all possible address for up to 8 different DS1631 devices
* @details   Each device found is stored in the _Devices array
* @param[in] i2cSpeed I2C Communications speed. If not specified defaults to I2C_STANDARD_MODE (100KHz)
...
uint8_t DS1631_Class::begin(const uint32_t i2cSpeed)
{
  ...
  for (uint8_t i=DS1631_MIN_ADDRESS;i<(DS1631_MIN_ADDRESS+DS1631_MAX_DEVICES);i++) // Loop all possible addresses
  {
...
*******************************************************************************************************************/
uint8_t DS1631_Class::readByte(const uint8_t device, const uint8_t command)
{
  Wire.beginTransmission(device);               // Address the I2C device 
...
* @return    2 bytes read
*******************************************************************************************************************/
uint16_t DS1631_Class::readWord(const uint8_t device, const uint8_t command)
{
  Wire.beginTransmission(device);               // Address the I2C device
  Wire.write(command);                          // Send register address to read
  _TransmissionStatus = Wire.endTransmission(); // Close transmission
...
*******************************************************************************************************************/
uint8_t DS1631_Class::writeByte(const uint8_t device, const uint8_t command, const uint8_t data )
{
  Wire.beginTransmission(device);               // Address the I2C device
  Wire.write(command);                          // Send register address to read
...
* @return    Transmission status, 0 on success otherwise I2C Error Code
*******************************************************************************************************************/
uint8_t DS1631_Class::writeWord(const uint8_t device, const uint8_t command, const int16_t data ) 
{
  Wire.beginTransmission(device);               // Address the I2C device
  Wire.write(command);                          // Send register address to read
...
* @details  If no device is specified then device[0] is used. The DS1631 returns raw values in 0.0625°C increments. 
*           A check is done to see if there is still an active conversion being processed and will wait until any 
*           conversion is complete.
* @param[in] device Device number
* @return    Temperature in units of 0.0625 degrees Celsius
*******************************************************************************************************************/
int16_t DS1631_Class::readTemp(const uint8_t device) 
{
  int16_t returnTemperature = 0;
  if (device>=_thermometers) return INT16_MIN; // Return error value if the device is out of bounds
  if (readByte(_Devices[device], DS1631_ACCESS_CONFIG) & 1) // If we are in one-shot mode then wait for conversion
  {
...
* @details  This corresponds to increments of 0.5°C, 0.25°C, 0.125°C and 0.0625°C and conversion times of 94ms, 
*           188ms, 375ms and 750ms respectively.
* @param[in] device Device number
* @param[in] precision 9, 10, 11 or 12 for number of bits precision to use. Invalid values are ignored
*******************************************************************************************************************/
void DS1631_Class::setPrecision(const uint8_t device,const uint8_t precision)
{
...
/***************************************************************************************************************//*!
* @brief    set the 16 bit value for the temperature alarm on a device and also reset the give alarm
* @details  A 0 in "alarmType" equates to low, any other value to high alarm. The alarmTemp is specified in internal 
*           units, that is in increments of 0.0625°C
* @param[in] device Device number
...
*******************************************************************************************************************/
void DS1631_Class::setAlarmTemperature(const uint8_t device, const uint8_t alarmType, int16_t alarmTemp)
{
  ...
  if (device<_thermometers)  // Skip if device is out of bounds
  {
    if (alarmType==0) // Branch here if setting low alarm
    {
      ...
      writeByte(_Devices[device],DS1631_ACCESS_CONFIG,readByte(_Devices[device],DS1631_ACCESS_CONFIG)&~0x20); 
    }
    else // Branch here if setting high alarm
    {
      writeWord(_Devices[device],DS1631_ACCESS_TH,alarmTemp); // Set the high temperature
      // Reset the indicator flag
      writeByte(_Devices[device],DS1631_ACCESS_CONFIG,readByte(_Devices[device],DS1631_ACCESS_CONFIG)&~0x40);
...
* @details   When an out-of-range device number is specified the function returns a UINT8_MAX value
* @param[in] device Device number
* @return    1 when the device has an active LOW alarm, 2 when the HIGH alarm is active, otherwise returns 0
*******************************************************************************************************************/
uint8_t DS1631_Class::getAlarm(const uint8_t device)
{
...
* @details   If the optional "continuous" boolean parameter is set to false, it will turn off continuous and 
*            activate one-shot measurements
* @param[in] device Device number
* @param[in] continuous Defaults to "true" if not specified, if set to false then one-shot measurements are done
*******************************************************************************************************************/
