...
*
* Example program for using the DS1631 library which allows access to the DS1631 I2C thermometer. The library as
...
*
* The datasheet for the DS1631 can be found at https://datasheets.maximintegrated.com/en/ds/DS1631-DS1731.pdf
*
...
* ------- | ---------- | ----------------------------- | ---------------------------------------------------
* 1.0.1   | 2019-02-03 | https://github.com/SV-Zanshin | Issue #7 - convert documentation to Doxygen
...
void setup()
{
  ...
  for (uint8_t i=0;i<DS1631.thermometers;i++) // loop for each thermometer
  {
    ...
    DS1631.setAlarmTemperature(i,1,DS1631.readTemp(i)+16); // Trigger alarm when temp goes up 1°C
...
void loop()
{
  for (uint8_t i=0;i<DS1631.thermometers;i++)
  {
    ...
    Serial.print(F(" = "));
    Serial.print(DS1631.readTemp(i)*0.0625,4); // convert internal to Celsius units using floating point
